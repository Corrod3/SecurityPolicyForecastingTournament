...
The research design aims at identifying different factors for individual forecasting success. For this, measurable differences between the individuals are a prerequisite. In the case of the forecasting tournament, this can be varified by checking the distribution of Brier scores. <!-- Cross reference to the figure --> Since the distribution ranges from `r round(min(SPFT$brier.avg),2)` to `r round(max(SPFT$brier.avg),2)` with a mean score of `r round(mean(SPFT$brier.avg), 2)`this condition is given.
...
Before turning to the hypotheses, it makes sense to see how the participants performed in comparison to a simple statistical benchmark. This gives us a picture of the overall performance of the forecasting crowd. A standard benchmark for this is the comparison of the forecasting scores to a situation with uniformly random distributed  outcomes [@Mellers.2015]. The t-test can be used to compare average brier score with actual outcomes to average brier scores with random outcomes.^[In this context a one-sided paired t-test is used to see whether the forecasters performed better: $H_0: \bar b =b_rand$ and $H_A: \bar b  <b_{rand}$.] The average Brier score with random events is `r round(mean(SB.R$brier.avg),digits=2)`, while the actual average Brier score of the participants was `r round(mean(SPFT$brier.avg),digits=2)` (`r t.test.against.random`).
...
Having established the performance of the crowd, the focus can now put on factors behind the forecasting success of individual forecasters. Starting with the dispositional factors. For intelligence, the forecasters mean score at the Berlin Numeracy Score was 2.05 (at a range from 0 to 4), which is slightly above the 1.6 average score @Cokely.2012 found for Berlin university students.^[The forecasting tournament used the four item 'paper and pencil' version of the test. For this version there is no general population norm score available. However, in principle it would be possible by adjusting the score to the  to the adaptive test format scores.] As the distribution of BNT scores illustrates <!-- cross referenc -->, the test is able to discriminates between the participants. It assigned the forecasters to five score levels, each of which is roughly equal in size.
...
*a. As expected the correlation is negative, which implies that more intelligent individuals tended to be more accurate forecasters. Hence, we can confirm our first hypothesis (1a). b. The correlation is not as expected as a better BNT score is not related to more forecasting accuracy. Hence, we cannot confirm the first hypothesis. There are several possible reasons for this. First, the hypothesed relation could be wrong. This is, however, rather unlikely, as it has been shown before in more sophisticated research designs [@Mellers.2015; @Poore.2014]. Second, the test could be not valid and therefore not measure what it is supposed to. Again, this is rather unlikely as it has been extensively tested [@Cokely.2012] but cannot fully be ruled out. Third and most likely: The forecasting success in this tournament rather a result of luck than skill. Since this was a one-off event where the forecasters had little chances of learning, forecasting skill might not have cristalized in the results and might need a more long-term setting. It does, hwoever, imply that the results for the other hypotheses should also be read with care.* <!-- add comment on significance --> *a. The significance of the finding further underlines the point. b. Since the results are not significance, not final conclusion can be made.*

```{r results="asis", echo = F}
cor.plot
```

The second dispositional factor under scrutiny was moral competency. The MCT scores of the participants ranged from `r round(min(SPFT$mct.c),2)` to `r round(max(SPFT$mct.c),2)`. Similar ranges were also found in other groups [@Lind.2008]. Since the completion of the moral competency test questions were not obligatory, for some participants it was not possible to compute a MCT score.^[However, this was not communicated to the participants and the missing values are rather due to forgotten question than due to people categorically not responsing to the moral competency test questions.] Again, we use the Pearson test to look at the relationship between forecasting accuracy and moral competency: `r cor.brier.mct`.
...
In Hypothesis 2 the role of decision-making time for forecasting accuracy was tested. 
