...
The research design aims at identifying different factors for individual forecasting success. For this, measurable differences between the individuals are a prerequisite. In the case of a forecasting tournament, this can be verified by looking at the distribution of Brier scores (Figure \ref{fig:brier}). Since the distribution ranges from `r round(min(SPFT$brier.avg),2)` to `r round(max(SPFT$brier.avg),2)` with a mean score of `r round(mean(SPFT$brier.avg), 2)` we have enough variance for further testing. 

```{r echo = FALSE, fig.cap="\\label{fig:brier} Brier score distribution", out.width=c('300px', '150px'), fig.align='center', fig.pos = 'H'}
brier.plot
```
...
<!--	[evaluation results] --> Hence, the forecasting crowd performed significantly better than the benchmark. This supports the view of forecasting optimists: To some degree forecasting seems possible.
...
Having this in mind, I can now turn to the second dispositional factor under scrutiny: moral competency. The Moral Competency Test (MCT) resulted in scores ranging from `r round(min(SPFT$mct.c, na.rm = TRUE),2)` to `r round(max(SPFT$mct.c, na.rm = TRUE),2)`. Similar ranges were also found in other groups [@Lind.2008]. The moral competency score is not available for all participants since the completion of the moral competency test questions were not obligatory. However, the missing values are largely due to forgeting to answer a sub-question than to categorical non-replies. ^[There is no procedure for calculating the MCT score with missing answers.] Hence, the scores should not have systematic non-resonse bias. In order to assess the hypothesis, the Pearson coefficient is used to evaluate the relationship between forecasting accuracy and moral competency: `r cor.brier.mct`.
...
<!-- Discussion of second methid to test the relevance of time: Divide participants into two groups depending on the time they spend in the questions and compare group means. The exact devision time is however arbitrary.-->

<!-- evaluation of Hypothesis 2 --> 
The negative correlation indicates that more time spend on answering the question is related to more forecasting accuracy. Moreover, the logarithmized time has a higher Pearson correlation coefficient than the time (`r round(cor(SPFT$brier.avg, SPFT$time.fq.sec.log, use="complete.obs"),2) > cor.brier.time.linear`) supporting hypothesis 2. Therefore, forecasters using more time for forecasting perform better and the added value decreases with time. 
...
```
<!-- Hypothesis 3 -->
Finally, hypothesis 3 was about the effect of an decision aid intervention. The participants were randomly assigned to a treatment (n = `r plyr::count(SPFT$Group[SPFT$Group == "Treatment"])[2]`) or control group (n = `r plyr::count(SPFT$Group[SPFT$Group == "Control"])[2]`). The treatment group was presented with a analytic guide, while the control group was not. To test whether the analytic guide had any impact on the forecasting accuracy the mean Brier scores of both groups is used. The mean Brier score of the treatment group is `r round(t.test.intervention[[5]][1],2)` and of the control group `r round(t.test.intervention[[5]][2],2)`. 

<!-- evaluation hyptothesis 3 -->
There is hardly any difference between the two groups (`r t.test.intervention.result`). From this we can take that the intervention did not lead to its intended impact and we cannot support the intervention hypothesis (3). Hence, a short decision guide is not an effective tool to improve forecasting accuracy. It can be added, just like list the hypothesis or map the evidence methodlogy [@Kretz.2015] to a list of decision aids with no to limited effect for analytical decisions. However, research on debiasing methods and decision aids still is in its infancy and no consensus has emerged yet in the literature. In this specific case the reason of failure are manifold:  First, the decision guide might have provided them with irrelevant information for their decision. If we believe in the research results by @Tetlock.2015 this should not be the case as so-called superforecasters used the approach descibed in the guide and fared rather well with it in the Good Judgement Project. Second, participants might have ignored the decision guide. This might in particular be true for forecasters who only spend a few minutes on answering the questions. The treatment included a check box where the respondents had to indicate whether they have read the guide in order to increase the attention to the treatment. However, applying decision methodologies requires cognitive effort of decision makers [@Kretz.2015, p. 68ff] and disrupts the train of thoughts <!-- quote Hernandez, I., & Preston, J. L. (2013). Disfluency disrupts the confirmation bias. Journal of Experimental Social Psychology, 49(1), 178-182. doi: http://dx.doi.org/10.1016/j.jesp.2012.08.010-->. When faced with difficult analytical tasks, like forecasting questions, decision makers are likely to use their mental capacities for information processing rather then to thing about their methodological approach.  
