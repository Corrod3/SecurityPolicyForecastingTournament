---
title: "Improving Forecasting for Foreign Policy ^[A summary of the forecasting tournament and its results are available on the [project website](https://corrod3.github.io/SecurityPolicyForecastingTournament/). Additional background information and methodlogical details can be access via the online appendix[LINK]. The script files are available on the author's [Github account](https://github.com/Corrod3/SecurityPolicyForecastingTournament) for replication. For the data please contact the author.]"
...
Ideally, we would like to predict the forecasting success of individuals without knowledge about their personal opinions. This is, on the one hand, for practical reasons, as it is both unfeasible to ask each forecaster every time about their personal view and, on the other hand, undesirable as forecasters should not be forced to reveal their personal opinions in a politicized environment. Moreover, if shown to be correct, this would imply ideal forecasters not to have opinions. This is neither desirable nor likely to be correct. 
Instead, as Tetlock already suggested, it is more plausible to ask whether individuals can separate their moral judgement from their analytical assessment. 
...
However, for the forecasting tournament in this paper the focus will be rather on minimal interventions as the scale of the tournament is rather small. It is therefore more suited to test mild interventions aiming at debiasing. This could for example be achieved by providing forecasters with analytical tools. @Kretz.2015 has done some research on mild decision aid interventions and found that most analysts tend to disregard decision aids, at least after some time. Kretz sees the reason for this in the additional mental capacities needed to apply the decision aids, which distracts the analysts from the actual problem at hand.^[[NOTE]Limits of de-biasing: http://sourcesandmethods.blogspot.de/2014/03/reduce-bias-in-analysis-why-should-we.html]  In his research only the mildest intervention proved to have a significant effect on the judgement quality. Following this idea, the paper tests a minimal intervention in the context of forecasting and tests the following:
...
<!-- [Participants] -->
In total 231 individuals participated in the security policy forecasting tournament. However, only 214 responses as for some participants there were signs of not taking the survey serious (failing attention checks, unrealistically short time used for participation) and others only submitted their forecast after February 12th, 2017.

<!--Setting -->
The security policy forecast tournament was implemented with the survey tool Qualtrics. 
...
The research design aims at identifying different factors for individual forecasting success. The distribution of Brier scores, which measured the individual success, indicates that forecasting skills among the participants of the security policy forecasting tournament varied.
...
Before turning to the hypotheses, we can test how the participant performed in comparison to a simple statistical benchmark. This gives us a picture of the overall performance of the forecaster crowd and is an empirical piece for the initially mentioned debate between the forecasting optimists and pessimists. A standard benchmark for this is the comparison to a random uniform distribution of outcomes [@Mellers.2015]. With the help of the t-test the average brier score of forecasts is compared to the average of forecasting scores when the outcomes are randomly distributed.^[In this context a one-sided t-test is used to see whether the forecasters performed better: $H_0: \bar b =b_rand$ and $H_A: \bar b  <b_{rand}$.]    The average Brier score for random events would be XX, the average Brier score of the forecasting tournament participants was XX (t(df) = XX).

<!--	[evaluation results] -->
*a. Clearly, the forecasters performed better than just random guessing.*
*b. The result indicates that on average the forecasters performed [just as well as / worse than] a random guess. This is consistent with prior findings on intuitive and expert judgements. Therefore, we need to look at the reasons for some forecasters to outperform the group average and the random guess benchmark.*

Standard benchmark for accuracy (Mellers et al. 2015) > measure against random uniform distribution of outcomes

<!--[Alternative measure: Proportions of forecasts on the correct side of 50%] -->
A second method to test the accuracy of the forecasting crowd is to measure the proportion of questions where there forecasters were on the correct side of 50% [@Mellers.2015, p. 6]. The perfect score would be 100%, a score corresponding to chance would be 50%. The forecasting average of the tournament participants was XX%. Again, we can use the t-test to measure the significance of the difference (t(213) = XX).
...
*a)	Like the first benchmark the shows that forecasters are significantly better than random guessing.*
*b)	Like the first benchmark the alternative measure shows that forecasters perform on average [like / worse] than random guessing.*

<!--	[Evaluation of Hypothesis] -->
Let’s start with looking at the hypotheses. What is the relationship between the dispositional factors intelligence and moral competency to forecasting accuracy? In the descriptive statistics table the distribution of the variables among the forecasters can be seen. The mean score for the Berlin Numeracy Score was 2.05 on a range from 0 to 4. 
