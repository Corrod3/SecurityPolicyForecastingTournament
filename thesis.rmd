...
The research design aims at identifying different factors for individual forecasting success. For this, measurable differences between the individuals are a prerequisite. In the case of the forecasting tournament, this can be varified by looking at the distribution of Brier scores. <!-- Cross reference to the figure --> Since the distribution ranges from `r round(min(SPFT$brier.avg),2)` to `r round(max(SPFT$brier.avg),2)` with a mean score of `r round(mean(SPFT$brier.avg), 2)`this condition is fulfilled
...
Before turning to the hypotheses, it makes sense to see how the participants performed in comparison to a simple statistical benchmark. This gives us a picture of the overall forecasting ability of the participants. A standard benchmark for this is the comparison of the realized forecasting scores to a situation with uniformly random distributed  outcomes [@Mellers.2015]. Whether the participants performed better than a random guess can be evaluated with a t-test.^[In this context a one-sided paired t-test is used to see whether the forecasters performed better: $H_0: \bar b =b_rand$ and $H_A: \bar b  <b_{rand}$.] The average Brier score with random events is `r round(mean(SB.R$brier.avg),digits=2)`, while the actual average Brier score of the participants was `r round(mean(SPFT$brier.avg),digits=2)` (`r t.test.against.random`).

<!--	[evaluation results] -->
*a. Clearly, the forecasters performed better than just random guessing. This supports the view of forecasting optimists: To some degree forecasting seems possible.*
*b. The result shows that on average forecasters did not perform better than a random quess[, they even performed worse]. This is not a surprise as it is in line with the literature on intuitive and expert judgements. Rather than looking at the group average we should look at successful forecasters and understand why they outperform the group average.*

<!--[Alternative measure: Proportions of forecasts on the correct side of 50%] -->
A second method to test the accuracy of the forecasting crowd is to measure the proportion of questions where  forecasters were on the correct side of 50% with there forecast [@Mellers.2015, p. 6]. The perfect score would be 100%, a score corresponding to chance 50%. The forecasting average of the tournament participants was `r round(mean(SB.CS$cs.avg)*100, 1)`%. Again, we can use the t-test to measure the significance of the difference (`r t.test.correct.side`).
...
*a.	Like for the first benchmark, this measurement also finds forecasters to perform better than random guessing.*
*b.	Like the first benchmark, the alternative measure shows that forecasters perform on average [like / worse] than random guessing.* Since the measure tries to capture the same thing, this is not surprising.
...
Having established the performance of the crowd, the focus can now turn to factors behind the forecasting success of individual forecasters. Starting with the first dispositional factor: intelligence. The forecasters mean score at the Berlin Numeracy Score was 2.05 (at a range from 0 to 4), which is slightly above the 1.6 average score @Cokely.2012 found for Berlin university students.^[The forecasting tournament used the four item 'paper and pencil' version of the test. For this version there is no general population norm score available. However, in principle it would be possible by adjusting the score to the  to the adaptive test format scores.] As the distribution of BNT scores illustrates <!-- cross referenc -->, the test is able to discriminates between the participants. It assigned the forecasters to five score levels, each of which is roughly equal in size.

```{r echo = FALSE, fig.cap="BNT score distribution", out.width=c('200px', '120px'), fig.align='center', fig.pos = 'H'}
bnt.plot
```

In order to understand the relationship between intelligence and forecasting accuracy, the Pearson correlation coefficient is a useful indicator. It gives us the direction of the relation, indicated by the sign (+ or -), and the magnitute of the effect on a range from 0 to 1. The correlation between the BNT score and the Brier score is: `r cor.brier.bnt`. The t-test checks whether the result is significantly different from no relationship between both variables ($r = 0$).
\footnote{In this case the following t-test statistic is used: $t = r\sqrt{\frac{n-2}{1-r^2}}$. Note that the Pearson correlation test treats the BNT score as a continuous variable, implying that the intelligence difference between the scores is about equal.}
*a. As expected the correlation is negative, which implies that more intelligent individuals tended to be more accurate forecasters. Hence, we can confirm our first hypothesis (1a) which is in line with prior findings in the literature. b. The correlation is not as expected as a better BNT score is not related to more forecasting accuracy. Hence, we cannot confirm the first hypothesis (1a). There are several possible reasons for this. First, the hypothesed relation could be wrong. This is, however, rather unlikely, as it has been shown before in more sophisticated research designs [@Mellers.2015; @Poore.2014]. Second, the test could be not valid and therefore not measure what it claims  to. Again, this is rather unlikely as it has been extensively tested [@Cokely.2012] but it cannot fully be ruled out that the risk literacy it measures is not relevant for forecasting. Third and most likely: The forecasting success in this tournament rather a result of luck than skill. Since this was a one-off event where the forecasters had little chances of learning, forecasting skill might not have cristalized in the results and might only be accessable in a more long-term setting. However, it does also imply that the results for the other hypotheses should also be read with care as chance might be the primary driver behind the forecasting scores.* <!-- add comment on significance --> *a. The significance of the finding further underlines the point. b. But due to the lack of significance, not final conclusion can be made.*

```{r results="asis", echo = F}
cor.plot
```

The second dispositional factor under scrutiny was moral competency. The MCT scores of the participants ranged from `r round(min(SPFT$mct.c),2)` to `r round(max(SPFT$mct.c),2)`. Similar ranges were also found in other groups [@Lind.2008]. Since the completion of the moral competency test questions were not obligatory, for some participants it was not possible to compute a MCT score.^[However, this was not communicated to the participants and the missing values are rather due to a missing responses to single questions than due to people categorically not responsing to the moral competency test questions.] Again, we use the Pearson test to look at the relationship between forecasting accuracy and moral competency: `r cor.brier.mct`.
...
```{r echo = FALSE, fig.pos='H', fig.cap= "Forecasting time - Brier score scatterplot"}
cor.brier.time.plot
```
