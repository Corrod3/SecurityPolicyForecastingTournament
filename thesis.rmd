...
The individual differences between forecasters can be used in different ways to combine forecasts. A simple approach, which was for example used by @Mellers.2014 in order to win the IARPA forecasting tournament, is to use only a subgroup of individuals for aggregation. If only forecasts of individuals who are expected to perform well are combined, the aggregated result reflects both individual forecasting skills and the wisdom of the crowd effect. The subgroup can be selected based on the past performance of forecasters, as for example @Mellers.2014 superforecasters. Since in this case the past performance of forecasters is unknown, this is not an option. However, we can use the predictors of forecasting success to select a subgroup. To illustrate the possibilities of aggregation, we can see how a group with at least had a minimum BNT score and spend sufficient time at the forecasting decision performed in comparison to the average forecast of the crowd: `r bs.cutoff.mean` to `r round(bs.mean,2)`. The improvement is not large, but there is one. Note that the choice of the cut off score is ultimately arbitrary and there is the problem of over-fitting as we exploit the same sample. But primary point here is to illustrate the potential of aggregation and if the factors of selecting the sub group are reliable predictors for forecasting accuracy, the aggregation approach should be applicable to other forecasting settings. 
