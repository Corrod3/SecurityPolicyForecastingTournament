...
date: "April 28, 2017"
abstract: In this paper, I explore the drivers of forecasting accuracy for geopolitical events with the help of a forecasting tournament. The paper analyses the responses of the participants in order to test and evaluate several explanations of successful forecasting. More specifically, it looks at (1) measurable characteristics of forecasters, (2) the decision environment in which a forecast is made and (3) minimal interventions aiming at improved forecasting judgements. My findings are that intelligence is a good indicator of forecasting success reinforcing prior findings on geopolitical forecasting. There is also some evidence that moral judgment competency is related to forecasting accurarcy, but further research is needed. Regarding the decision context, the forecasting tournament showed that more forecasting time is related to more accuracy, but with diminishing returns. Finally, the forecasting competition indicated that small interventions in form of analytical guides cannot improve forecasting judgements. These insights can be used to compute improved crowd forecasts. The results were derived from a security policy forecasting tournament which took place from February to April 2017 and which had more than 200 participants, comprised out of university students with a strong interest in the field, paid online respondents and voluntary online users.
output: 
  pdf_document:
    toc: true
...
This paper uses a test which was not used for forecasting accuracy yet: The Berlin Numeracy Test (BNT) by @Cokely.2012. It is a relatively new psychometric scale that was in particular developed to access statistical numeracy and risk literacy. This makes the test in particular suitable for forecasting decisions involving probability judgements. Since it is especially suited to differentiate between individuals with higher education, it is also likely to perform better than the numeracy scale used by @Mellers.2015 which lacked differentiation. Another reason for the BNT is its length: The test consists only of four questions, making it suitable for one time forecasting tournament. 
...
Having this in mind, I can now turn to the second dispositional factor under scrutiny: moral competency. The Moral Competency Test (MCT) resulted in scores ranging from `r round(min(SPFT$mct.c, na.rm = TRUE),2)` to `r round(max(SPFT$mct.c, na.rm = TRUE),2)`. Similar ranges were also found in other groups [@Lind.2008].<!-- change, if outlier continues to exist, to: Except for a few high outliers (>0.5) this reflects the pattern found in other groups. --> The moral competency score is not available for all participants since the completion of the moral competency test questions were not obligatory. However, the missing values are largely due to forgetting to answer a sub-question than to categorical non-replies. ^[There is no procedure for calculating the MCT score with missing answers.] Hence, the scores should not have systematic non-response bias. In order to assess the hypothesis, the Pearson coefficient is used to evaluate the relationship between forecasting accuracy and moral competency: `r cor.brier.mct`.
...
So what happened? To see whether the treatment lead to any behavioral change, we can check whether it had any effect on the time used for answering the forecasting questions. And this is the case, as the treatment group used `r round(mean(SPFT$time.fq.sec[SPFT$Group == "Treatment"], na.rm = TRUE),1)` min and the control group `r round(mean(SPFT$time.fq.sec[SPFT$Group == "Control"], na.rm = TRUE),1)` min. But these means might be the result of few extreme outliers. To compare whether the difference is more than a random occurence, the logarithmized times can be used.^[Eye-balling the data shows that time in minutes used by the forecasters clearly does not follow a Gaussian distribution, but the logarithmized distribution resembles the classical Gaussian curve.] Applying a t-test shows that the treatment group used more time if we use a significance level of 10%, but not for 5% (`r t.test.intervention.time`). Even though this is not a clear result, it does also not rule out a small behavioral change from the decision guide.
...
hypo3.time.plot + scale_fill_manual( values = c("gray","#C02F39"))
```
...
So far the focus of the paper was on the quality of individual forecasting decisions. However, having a group of forecasters does allow to use the wisdom of the crowd effect to generate forecasts. The simplest approach would be, like @Galton.1907 did in the case of predicting the weight of oxen, to use the average probability forecasts of the participants. Compared to the average Brier score among the participants (`r round(mean(SPFT$brier.avg),2)`) the  score from averaging the forecasts of each question is `r bs.mean`.<!--check aggregated result --> Obviously, the crowd forecast performed significantly better than average participant in the tournament (`r t.test.bs.avg.mean`).

<!-- 2. intro: smart aggregation -->
But if we take into account what we know about individual forecasting decisions, the crowd wisdom can be aggregated in a more effective way. On the one hand, we can use individual predictors of forecasting success to weight the individual judgements appropriately. On the other hand, extremizing aggregated forecasts can correct decision making biases in forecasting.

<!-- sub group aggregating-->
The individual differences between forecasters can be exploited in different ways for combining forecasts. A simple approach is to use only a subgroup of individuals for aggregation. If only forecasts of individuals who are expected to perform well are combined, the aggregated result reflects both individual forecasting skills and the wisdom of the crowd effect. The subgroup can be selected based on the past performance of forecasters, as for example done by @Mellers.2014. Since in the security policy forecasting tournament the past performance of forecasters is unknown, this is not an option. However, we can use the predictors of forecasting success to select a subgroup. Since the primary aim here is to illustrate the potential of aggregation, we can use the confirmed drivers of forecasting success: The BNT score and the time spend with forecasting.  Where exactly we make the cutoff is arbitrary, but it seems plausible drop the lower 50% for each variable in order to make the sample not too small and to ensure a group with decent forecasting skills.^[In this case the subgroup consists of individuals which at least hat a BNT score of 3 and spend more time than the median forecaster on the questions. The other predictors available in the forecasting tournament data are not used as they were not sufficiently supported (moral competency, intervention).]
In this case, the average forecasts of the subgroup has a Brier score or `r bs.cutoff.mean` compared to `r bs.mean` if the whole group is used.<!-- Evaluation subgroup aggregation --> The improvement is not large, but there is one. Theoretically, we could determine the ideal cutoff point for the subgroup and improve the aggregated forecast even more, but then we risk overfitting the data. 

<!-- weighted forecasts --> Another approach is to weight the forecasts with the drivers of forecasting success. This approach has the advantage to avoid a cutoff. If we assume intelligence and time interact as drivers of forecasting success, it is plausible to construct weights by multiplying the BNT score and the logaritmized time ($w_i = bnt_i \cdot log(time_i)$). Like for the sub group aggregation, the weighted forecast performs slightly better (`r bs.mean.w.bnt.time`) than the simple average (`r bs.mean`).

<!-- debiasing / extremizing -->
Apart from the individual differences, the aggregated forecast can also be extremized to reflect a more accurate forecast. There are two primary reasons for this [@Baron.2014]: First, the random errors are compressed at the ends of the 0 to 1 probability scale which pushes the average forecast towards 0.5. Second, forecasters do not utilize all available information and individual forecasters draw from different sources. Hence, the aggregated forecast ignores that forecasters give judgements based on information pointing into the direction of the best informed probability forecast and that the diversity of information sources allows us have more confidence into the aggregated than individual forecast. 

<!-- how to extremize -->
For the implementation of the extremization this work orients at @Satopaa.2014, who use a simple logit model to combine the forecasts. The logit transformation is convenient for extremizing as it maps the probabilties to a continuous domain where a 50% probability is equivalent to a log-odds value of 0.
<!--Theoretically this method is based on the idea that forecasters base their decision on partial information. For example, assume a case where the best informed forecast would be 0.9. Now a forecaster is asked about the likelihood of the event. Before looking at any information, the rational decision maker would assume 0.5 as a starting point for his / her considerations. Then the forecaster can incorporate relevant information into the judgement, which should lead to incremental adjustments of 0.5 in direction to 0.9. However, most forecasters will stop somewhere between full ignorance (0.5) and full information (0.9).^[$y_i = log\Big(\frac{p}{1-p}\Big)^{\frac{1}{a}} + \epsilon_i$]-->
The systematic bias, which we want to correct for with extremizing, can be described with a single variable $a \in [0,\infty]$. $a = 1$ describes the case when the individual forecasts are the best-informed forecasts, while $a > 1$ captures underconfidence of forecasters. In principle, this representation is also applicable for overconfidence ($a < 1$). But former research has shown that forecasting crowds as a whole tend be underconfident [@Baron.2014]. Mathematically, the relation between the best informed forecast ($p$) and the individual forecasts $p_i$ can be descibed as follows:

\begin{equation}
 y_i = log\Big(\frac{p_i}{1-p_i}\Big) = log\Big(\frac{p}{1-p}\Big)^{\frac{1}{a}} + \epsilon_i
\end{equation}

In order to illustrate the potential of extremizing, we have to compute the corrected probabilities $p_E$ for each question. This can be done with an MLE estimator:^[For a more in-depth explanation see @Satopaa.2014.]
...
To understand the systematic bias in the forecasting tournament we can find the $a$ which minimizes the Brier score. Here, this is `r round(bias.a$minimum,2)`. Hence, as expected the forecasting crowd is underconfident and if we would correct for the bias, the Brier score would be `r round(bias.a$objective,3)`(Figure \ref{fig:bias}). Hence, extremizing can lead to similar accuracy gains as exploiting the individual differences between forecasters.

```{r echo = FALSE, fig.pos='H', fig.cap= "\\label{fig:bias} Systematic bias and forecasting accuracy", out.width=c('300px', '200px'), fig.align='center'}
brier.ext.plot
```

<!--Another 
example question and illustration of aggregation.
...
However, there is also another theoretical explanation of the bias.
Lets assume, the best-informed probability of an event is 0.9 and a rational decision maker wants to find this probability. At the begining the forecaster would have no information and start with 0.5 as prior. Then the forecaster begins to incorporate information and this information gives him/her good reason to adjust the probability slowly upwards. However, before the forecaster has included all information and reached 0.9 

 Past research has shown that subjective probability forecasts tend to be underconfident [@Wallsten.1997, @Zhang.2012].
