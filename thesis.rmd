...
date: "April 28, 2017"
abstract: In this paper, I explore the drivers of forecasting accuracy for geopolitical events with the help of a forecasting tournament. The paper analyses the responses of the participants in order to test and evaluate several explanations of successful forecasting. More specifically, it looks at (1) measurable characteristics of forecasters, (2) the decision environment in which a forecast is made and (3) minimal interventions aiming at improved forecasting judgements. My findings are that intelligence is a good indicator of forecasting success reinforcing prior findings on geopolitical forecasting. I also find that moral judgment competency a less useful predictor than intelligence. Regarding the decision context, this research provides evidence that more forecasting time is related to more accuracy, but there is a diminishing return on time. Finally, the forecasting competition indicated that small interventions in form of analytical guides cannot improve forecasting judgements. In the end this paper illustrates how these insights can be used to aggregate individual forecasts more effectively. The results were derived from a security policy forecasting tournament which took place from February to April 2017 and which had more than 200 participants, comprised out of university students with a strong interest in the field, paid online respondents and voluntary online users.
output: 
  pdf_document:
    toc: true
...
The individual differences between forecasters can be exploited in different ways for combining forecasts. A simple approach is to use only a subgroup of individuals for aggregation. If only forecasts of individuals who are expected to perform well are combined, the aggregated result reflects both individual forecasting skills and the wisdom of the crowd effect. The subgroup can be selected based on the past performance of forecasters, as for example done by @Mellers.2014. Since in this case the past performance of forecasters is unknown, this is not an option. However, we can use the predictors of forecasting success to select a subgroup. Since the primary aim here is to illustrate the potential of aggregation, we can use the hypothesized and tested drivers of forecasting success to select the subgroup: The BNT score and the time spend with forecasting.  Where exactly we make the cutoff is arbitrary, but it seems plausible drop the lower 50% in order to make the sample not too small and to group with decent forecasting skills.^[In this case the subgroup consists of individuals which at least hat a BNT score of 3 and spend more time than the median forecaster on the questions. The other predictors available in the forecasting tournament data have not been supported sufficiently (moral competency, intervention).]
In this case the average forecasts of the subgroup has a Brier score or `r bs.cutoff.mean` compared to `r bs.mean` if the whole group is used.<!-- Evaluation subgroup aggregation --> The improvement is not large, but there is one. Theoretically we could determine the ideal cutoff point for the subgroup and improve the aggregated forecast even more, but then we will overfit the data. 

<!-- weighted forecasts --> Another approach is to weight the forecasts with the drivers of forecasting success. This approach has the advantage to avoid a cutoff. Against, since this is primarily for illustrative purposes, the weights are constructed by multiplying the BNT score and the logaritmized time ($w_i = bnt_i \cdot log(time_i)$). Like for the sub group aggregation, the weighted forecast performs slightly better (`r bs.mean.w.bnt.time`) than the simple average (`r bs.mean`).

<!-- debiasing / extremizing -->
Apart from the individual differences, known decision making biases can also be utilized for aggregration. For forecasting decision one bias is particular persistent: Overvaluing small probabilties and undervaluing high probabilities. This bias is basically the reason of the long-shot bias in forecasting markets<!-- and has been discussed in behavioral economics for some time-->. However, there is also another theoretical explanation of the bias. Lets assume, the "true" probability of an event is 0.9 and a rational decision maker wants to find this probability. At the begining the forecaster would have no information and start with 0.5 as prior. Then the forecaster begins to incorporate information and this information gives him/her good reason to adjust the probability slowly upwards. However, before the forecaster has included all information and reached 0.9 
...
<!-- how to extremize -->
In order to implement the extremization I use the logit model proposed by @Satopaa.2014. 

1. 

\begin{equation}
 y_i = log\Big(\frac{p}{1-p}\Big)^{\frac{1}{a}} + \epsilon_i
\end{equation}
2.
