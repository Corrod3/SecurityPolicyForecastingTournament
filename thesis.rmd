...
Having established the performance of the crowd, I can now look at factors behind the forecasting success of individual forecasters. Starting with the dispositional factors. In general, the forecasters mean score for the Berlin Numeracy Score was 2.05 on a range from 0 to 4, which is slightly above 1.6 average score @Cokely.2012 found for Berlin university students.^[The forecasting tournament used the four item 'paper and pencil' version of the test. In order to compare the score to the general population, it has to be adjusted to the adaptive test format scores.] As the distribution of BNT scores illustrates, the test is able to discriminates between the participants and assign them to five groups of roughly equal size depending on the score.
...
\footnote{In this case the following t-test statistic is used: $t = r\sqrt{\frac{n-2}{1-r^2}}$. Note that the Pearson correlation test treats the BNT score as a continuous variable, implying that the intelligence difference between the scores is about equal. If we assume different numeracy skills are about equally distributed among university students and the BNT allows to sort them in about equal sized groups this assumption is justifiable.}
*a. As expected the correlation is negative, which implies that more intelligent individuals tended to be more accurate forecasters. Hence, we can confirm our first hypothesis. b. The correlation is not as expected as a better BNT score is not related to more forecasting accuracy. Hence, we cannot confirm the first hypothesis. But this does not imply, that the hypothesis is wrong as it has already been replicated in several studies. More likely, this indicates a problem with the data sample in this research.*
